const {
  ...
  app.post("/api/login", async (req, res) => {
    try {
      ...
      const userResponse = await client.query(SQL, [username,password]);
      console.log(userResponse.rows[0]);
      const user = userResponse.rows[0];
      if (!user.length || !(await bcrypt.compare(password, user.password))) {
  ...
  const init = async () => {
    ...
    const [max, noel, john, joy, nathan] = await Promise.all([
      ...
      createUser({
        username: "joy",
        email: "johnnsssy@example.com",
        password: "password4",
      }),
      createUser({
        username: "nathan",
        email: "johnnydsa@example.com",
        password: "password5",
      }),
    ]);
    console.log(max);
